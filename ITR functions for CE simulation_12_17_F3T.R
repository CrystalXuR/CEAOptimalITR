...
cumC.gam = function(i, trt_i, k, betaC,gammaC,I.delta){
  tmpx = c(1,x1[i],x2[i],x3[i],x4[i],x5[i]) 
  tmp.beta = exp(t(betaC) %*% tmpx + gammaC*trt_i[i])
  
  inital.C = rgamma(1,shape = k,scale=10*tmp.beta)
  ongoing.C = rgamma(48, shape = k, scale=tmp.beta)
  ongoing.C = I.delta[i,]*ongoing.C
  ongoing.C = ifelse(ongoing.C==0, NA,ongoing.C)
  dying.C = rgamma(1, shape = k, scale=3*tmp.beta)                    # if one is censored, we don't add the dying cost=0
